...
#include<string.h>
	...
	int opc,opc2,opc3,num,i,x,N,NE,M,y,temp;
	...
	while(i<=NE);
		...
		do{
			...
			case 4:
				...
				temp = Emp.Clave[i];
				int j=i-1;
				if((Emp.Clave[i]>temp)&&(j>=0))
					Emp.Clave[j+1]=Emp.Clave[i];
					j--;
				j+1==temp;
				...
				int N,y;
				 ...
				 scanf("%d",&N);
				 f=fopen("C:\Users\Jana\Desktop\Estructuras II\datos.dat","rb");
				 fseek(f,sizeof(Emp.Clave),SEEK_END);
				 y=ftell (f);
				 if((N)*sizeof(Emp.Clave)>=y){
					 printf("No Existe el Numero\n");
				 }
				 else
				 {
					 printf("\n");
				 fseek (f,(N-1)*sizeof(Emp.Nombre),SEEK_SET );
				 fread(&Emp.Clave,sizeof(Emp.Nombre),1,f);
						printf("Clave\tNombre\tDepto\tSueldo\t\tTiempo\tPuesto\n");
						printf("%d\t",Emp.Clave);
						printf("%s\t",Emp.Nombre);
						printf("%d\t",Emp.Depto);
